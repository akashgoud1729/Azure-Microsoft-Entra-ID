resource "azuread_group" "engineering"{
    display_name = "Education_department"
...
resource "azuread_group" "managers"{
    display_name = "Education_managers"
    security_enabled = true
}

resource "azuread_group_member" "managers"{
    for_each = { for u in azuread_user.users: u.mail_nickname => u if u.job_title == "managers" }
...
resource "azuread_group_member" "engineers"{
    for_each = { for u in azuread_user.users: u.mail_nickname => u if u.department == "engineer" }
